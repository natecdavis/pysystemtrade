#!/usr/bin/env python3
"""
Sweep xs_val_weight values and compare backtest performance.

Runs a full backtest for each weight, collects metrics, and prints a
comparison table with adoption criteria check.

Signal: Cross-sectional ranking of AdrActCnt / CapMrktCurUSD (C-5 VAL factor).
Lit: Cong et al. (2022) C-5 VAL — crypto book-to-market. High on-chain activity
per dollar of market cap = undervalued relative to peers → LONG (+20).

Distinct from xs_activity (level): normalises by market cap, removing the
structural BTC/ETH dominance. A mid-cap chain with high usage relative to its
valuation outscores BTC/ETH. Anti-correlated with xs_activity (BTC/ETH rank
highest on xs_activity but lowest on xs_val) — provides diversification.
...
Prerequisites:
  data/active_addresses.parquet  (run scripts/download_active_addresses.py)
  data/market_cap.parquet        (run scripts/download_market_cap.py)

Usage:
    python scripts/sweep_xs_val.py \\
        --base-config config/crypto_perps_full_rules.yaml \\
        --data data/dataset_538registry_6yr_jagged.parquet \\
        --outdir out/xs_val_sweep \\
...
def print_comparison(results: list[dict], lookback: int) -> None:
    ...
    print()
    print('=' * 100)
    print('XS VAL WEIGHT SWEEP — RESULTS')
    print('Signal: Cross-sectional AdrActCnt/CapMrktCurUSD (C-5 VAL factor)')
    print('Lit: Cong et al. (2022) C-5 VAL — crypto book-to-market (addresses per $ of market cap)')
    print(f'Fixed params: lookback={lookback}d (EWM smoothing of ratio)')
    print('=' * 100)
    ...
    if candidates:
        ...
        print(f'  RECOMMENDATION: Use xs_val_weight={best["weight"]:.2f}')
        ...
        print(f'    xs_val_weight: {best["weight"]:.2f}')
        print(f'    xs_val_lookback: {lookback}')
    else:
        print(
            '  RECOMMENDATION: No weight passes all adoption criteria — '
            'keep xs_val_weight=0.0'
        )
        print()
        print('  Likely causes if all weights fail:')
        print('    - VAL ratio may be correlated with existing xs_activity sleeve')
        print('    - Market cap normalisation may reduce signal quality on illiquid instruments')
        print('    - Coverage (~42/300 instruments) limits aggregate Sharpe impact')
        print('    - Consider whether xs_activity and xs_val are too anti-correlated to combine')
    print()


def main():
    parser = argparse.ArgumentParser(
        description='Sweep xs_val_weight values.',
    ...
    parser.add_argument(
        '--outdir', type=Path,
        default=Path('out/xs_val_sweep'),
    )
    parser.add_argument(
        '--weights', type=float, nargs='+',
        default=[0.0, 0.2, 0.5, 1.0, 2.0],
        help='xs_val_weight values to test (default: 0.0 0.2 0.5 1.0 2.0)',
    )
    parser.add_argument(
    ...
    # Check both required data files are present
    ...
    mcap_candidate = args.data.parent / 'market_cap.parquet'
    if not mcap_candidate.exists():
        print(f'ERROR: market_cap.parquet not found at {mcap_candidate}')
        print('  Run: python scripts/download_market_cap.py')
        sys.exit(1)
    else:
        print(f'Market cap data:       {mcap_candidate} ✓')

    args.outdir.mkdir(parents=True, exist_ok=True)

    base_cfg = load_yaml(args.base_config)
    lookback = base_cfg.get('xs_val_lookback', 30)

    print(f'Base config:    {args.base_config}')
    print(f'Data:           {args.data}')
    print(f'Output dir:     {args.outdir}')
    print(f'Weights:        {args.weights}')
    print(f'Lookback:       {lookback} days (EWM smoothing of ratio, fixed)')
    print()
    ...
    for weight in args.weights:
        ...
        print(f'{"─" * 60}')
        print(f'Running: xs_val_weight = {weight:.2f}  →  {run_outdir}')
        ...
        cfg = dict(base_cfg)
        cfg['xs_val_weight'] = float(weight)
    ...
    summary_path = args.outdir / 'xs_val_sweep_summary.json'

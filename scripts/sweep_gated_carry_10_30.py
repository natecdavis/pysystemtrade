#!/usr/bin/env python3
"""
Sweep forecast_weight for gated_carry_10 and gated_carry_30 jointly,
keeping gated_carry_60 fixed at 0.10 (adopted 2026-03-07).

Motivation: empirical weight diagnosis (scripts/diagnose_forecast_weights.py) finds
gated_carry_10 and gated_carry_30 each deserve ~1.5× more weight than current 0.05.
The 10-day and 30-day smoothers share a correlated carry signal so sweeping them
jointly is more conservative than sweeping independently.

Fixed weights:
  gated_carry_60: 0.10  (just adopted)
  All other rules: unchanged from base config

Sweep range: [0.05, 0.07, 0.08, 0.10]
  0.05 = current baseline (equal to pre-adoption carry_10/30)
  0.07 = modest increase (~1.4× baseline)
  0.08 = near empirical estimate (~1.6× baseline)
  0.10 = same as carry_60, upper bound (2× baseline)
...
Usage:
    python scripts/sweep_gated_carry_10_30.py \\
        --base-config config/crypto_perps_full_rules.yaml \\
        --data data/dataset_538registry_6yr_jagged.parquet \\
        --outdir out/gated_carry_10_30_sweep
"""
...
RULES_TO_SWEEP   = ['gated_carry_10', 'gated_carry_30']
FIXED_CARRY_60   = {'gated_carry_60': 0.10}
DEFAULT_WEIGHTS  = [0.05, 0.07, 0.08, 0.10]
...
def effective_shares(w10_30: float, base_cfg: dict) -> tuple[float, float, float]:
    """
    Return (c10_pct, c30_pct, total_carry_pct) as normalised % of all forecast_weights.
    """
    ...
    if total == 0:
        return 0.0, 0.0, 0.0
    c10_pct  = fw.get('gated_carry_10', 0) / total * 100
    c30_pct  = fw.get('gated_carry_30', 0) / total * 100
    c60      = fw.get('gated_carry_60', 0)
    carry_total = fw.get('gated_carry_10', 0) + fw.get('gated_carry_30', 0) + c60
    carry_pct   = carry_total / total * 100
    return c10_pct, c30_pct, carry_pct


def print_comparison(results: list[dict], base_cfg: dict) -> None:
    print()
    print('=' * 115)
    print('GATED_CARRY_10/30 FORECAST WEIGHT SWEEP — RESULTS')
    print(f'Fixed: gated_carry_60=0.10. carry_10 and carry_30 varied jointly. All other weights unchanged.')
    print('=' * 115)

    hdr = (
        f'{"c10/30 w":>9}  {"c10%":>6}  {"c30%":>6}  {"carry%":>7}  '
    ...
    for r in results:
        ...
        c10_pct, c30_pct, carry_pct = effective_shares(w, base_cfg)
        ...
        print(
            f'{w:>9.2f}  {c10_pct:>5.1f}%  {c30_pct:>5.1f}%  {carry_pct:>6.1f}%  '
    ...
    print('ADOPTION CRITERIA (vs baseline at carry_10_30_weight=0.05):')
    ...
    for r in results[1:]:
        ...
        print(
            f'  carry_10_30_weight={r["carry_10_30_weight"]:.2f}:  '
    ...
    if candidates:
        ...
        w    = best['carry_10_30_weight']
        print(f'  RECOMMENDATION: Set gated_carry_10: {w:.2f} and gated_carry_30: {w:.2f} in forecast_weights')
        print(f'  (gated_carry_60 remains at 0.10)')
    else:
        print('  No weight passes all adoption criteria — keep gated_carry_10/30: 0.05 (current)')
    print()


def main():
    parser = argparse.ArgumentParser(
        description='Sweep gated_carry_10 and gated_carry_30 forecast_weights jointly (carry_60 fixed at 0.10).',
    ...
    parser.add_argument('--outdir',      type=Path,
                        default=Path('out/gated_carry_10_30_sweep'))
    parser.add_argument('--weights',     type=float, nargs='+',
                        default=DEFAULT_WEIGHTS,
                        help=f'carry_10/30 weights to test (default: {DEFAULT_WEIGHTS})')
    ...
    for w in args.weights:
        tag       = f'c10_30_{w:.2f}'.replace('.', 'p')
        run_outdir = args.outdir / tag

        print(f'{"─" * 70}')
        c10_pct, c30_pct, carry_pct = effective_shares(w, base_cfg)
        print(
            f'Running: gated_carry_10 = gated_carry_30 = {w:.4f}  '
            f'(carry_10={c10_pct:.1f}%, carry_30={c30_pct:.1f}%, total carry={carry_pct:.1f}%)  '
            f'→ {run_outdir}'
        )
        ...
        for rule in RULES_TO_SWEEP:
            cfg['forecast_weights'][rule] = float(w)
        for rule, val in FIXED_CARRY_60.items():

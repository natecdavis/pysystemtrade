#!/usr/bin/env python3
"""
Sweep MVRV cycle filter parameters (overbought_threshold × min_scale grid).

Runs a full backtest for each combination, collects metrics, and prints a
comparison table including ΔSharpe, ΔCalmar, ΔMaxDD, and firing_pct (% days
where MVRV exceeds the threshold).
...
Adoption criteria:
  ...
  - Calmar non-monotone across threshold grid (proves genuine signal)

Usage:
    python scripts/sweep_mvrv_params.py \\
        --base-config config/crypto_perps_full_rules.yaml \\
        --data data/dataset_538registry_6yr_jagged.parquet \\
        --outdir out/mvrv_sweep
    ...
    python scripts/sweep_mvrv_params.py --outdir out/mvrv_sweep --skip-existing
"""
...
def compute_firing_pct(mvrv_path: Path, threshold: float) -> float:
    """Compute fraction of days where MVRV > threshold (over full history)."""
    try:
        df = pd.read_parquet(mvrv_path)
        mvrv = df['mvrv_ratio'].astype(float)
        return float((mvrv > threshold).mean() * 100)
    except Exception:
        return float('nan')


def print_comparison(results: list, mvrv_path: Path) -> None:
    """Print formatted comparison table."""
    print()
    print('=' * 120)
    print('MVRV CYCLE FILTER SWEEP — RESULTS')
    print('=' * 120)

    hdr = (
        f'{"Threshold":>10}  {"MinScale":>8}  {"Sharpe":>8}  {"Calmar":>8}  {"CAGR":>8}  '
        f'{"Vol":>8}  {"MaxDD":>8}  {"Crisis Ret":>10}  {"Fire%":>7}  {"ΔSharpe":>8}  {"ΔCalmar":>8}  {"ΔMaxDD":>8}'
    )
    print(hdr)
    print('─' * 120)
    ...
    for r in results:
        m = r.get('metrics', {})
        threshold = r['overbought_threshold']
        ...
        firing_pct = r.get('firing_pct', float('nan'))
        ...
        fire_str = f'{firing_pct:.1f}%' if not pd.isna(firing_pct) else '  N/A'
        print(
            f'{threshold!s:>10}  '
            f'{min_scale:>8.2f}  '
            f'{sharpe:>8.4f}  '
    ...
    print('  Sharpe improvement ≥ +1%  |  MaxDD worsening < +3pp absolute  |  Calmar non-monotone (genuine signal)')
    print()
    candidates = []
    calmar_values = []
    for r in results[1:]:  # skip baseline
        ...
        print(
            f'  th={r["overbought_threshold"]}, scale={r["min_scale"]:.2f}:  '
    ...
    if candidates:
        ...
        print(
            f'  RECOMMENDATION: Use overbought_threshold={best["overbought_threshold"]}, '
            f'min_scale={best["min_scale"]:.2f}'
        )
        print(f'  (highest Sharpe among combinations that pass all criteria)')
        print(f'  Set use_mvrv_overlay: true in config after validation.')
    else:
        print('  RECOMMENDATION: No parameter combination passes all adoption criteria — keep use_mvrv_overlay: false')
    print()


def main():
    parser = argparse.ArgumentParser(
        description='Sweep MVRV cycle filter parameters.',
    ...
    parser.add_argument(
        '--outdir', type=Path,
        default=Path('out/mvrv_sweep'),
    )
    parser.add_argument(
        '--thresholds', type=float, nargs='+',
        default=[2.5, 3.0, 3.5],
        help='overbought_threshold values to test (default: 2.5 3.0 3.5)',
    )
    parser.add_argument(
    ...
    # Check MVRV data is present
    mvrv_candidate = args.data.parent / 'mvrv_index.parquet'
    if not mvrv_candidate.exists():
        print(f'ERROR: mvrv_index.parquet not found at {mvrv_candidate}')
        print('  Run: python scripts/download_mvrv_index.py')
        sys.exit(1)
    ...
    print(f'MVRV data:    {mvrv_candidate}')
    print(f'Output dir:   {args.outdir}')
    print(f'Thresholds:   {args.thresholds}')
    print(f'Min scales:   {args.scales}')
    print(f'Total runs:   {n_runs} (1 baseline + {n_runs - 1} parameter combinations)')

    # Show firing percentages for context
    print('\nFiring percentages (% of all MVRV history days above threshold):')
    for th in args.thresholds:
        pct = compute_firing_pct(mvrv_candidate, th)
        print(f'  MVRV > {th}: {pct:.1f}% of days')
    print()

    results = []

    # ── Baseline: use_mvrv_overlay=false ─────────────────────────────────────
    print(f'{"─"*60}')
    print('Running baseline (use_mvrv_overlay=false)')
    ...
    result['overbought_threshold'] = 'N/A'
    result['min_scale'] = float('nan')
    result['firing_pct'] = float('nan')
    ...
    for threshold in args.thresholds:
        for min_scale in args.scales:
            ...
            print(f'{"─"*60}')
            print(f'Running: overbought_threshold={threshold}, min_scale={min_scale:.2f}  →  {run_outdir}')

            if args.skip_existing and (run_outdir / 'performance_summary.json').exists():
                print('  SKIP (already done)')
            else:
                cfg = copy.deepcopy(base_cfg)
                cfg['use_mvrv_overlay'] = True
                cfg['mvrv_overlay_params'] = {
                    'overbought_threshold': threshold,
            ...
            result['overbought_threshold'] = threshold
            result['min_scale'] = min_scale
            result['firing_pct'] = compute_firing_pct(mvrv_candidate, threshold)
            result['label'] = ''
            results.append(result)

    # ── Print comparison ──────────────────────────────────────────────────────
    print_comparison(results, mvrv_candidate)

"""
Ablation backtest for OI trend and vol TS signals against the flat-65 baseline.

Tests each rule individually as the 66th rule (all 66 at equal weight 1/66).
Baseline: flat-65 at Sharpe=1.4431, Calmar=2.3929, MaxDD=-5.97%

Rules tested:
  xs_oi_trend:   EWMAC(30,120) on log(OI), XS-ranked — directional OI accumulation
  vol_trend_16:  EWMAC(16,64) on per-instrument realized vol, inverted
  vol_zscore_ts: Per-instrument RV z-score vs own 252d history, inverted

Usage:
  python scripts/run_oi_vol_ablation.py
"""
...
logger = logging.getLogger("oi_vol_ablation")

_RULES = ["xs_oi_trend", "vol_trend_16", "vol_zscore_ts"]
_BASELINE = {"sharpe": 1.4431, "calmar": 2.3929, "maxdd": -0.0597}
...
def run_ablation(
    ...
    out_dir: str = "out/oi_vol_ablation",
):
    ...
    print(f"\n{'='*70}")
    print(f"OI TREND + VOL TS ABLATION RESULTS  (flat-65 baseline: "
...
def main():
    ...
    parser.add_argument("--outdir", default="out/oi_vol_ablation")

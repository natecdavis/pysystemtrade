"""
Ablation backtest for three macro direction trading rules against the flat-63 baseline.

Tests each rule individually as the 64th rule (all 64 at equal weight 1/64 = 0.015625).
Baseline: flat-63 at Sharpe=1.3983, Calmar=1.9356, MaxDD=-7.25%

Rules tested:
  macro_momentum_16: OLS fitted-value EWMAC(16,64) — complement to residual_momentum_16
  macro_momentum_32: OLS fitted-value EWMAC(32,128) — complement to residual_momentum_32
  dxy_momentum_16:   Direct DXY EWMAC(16,64) inverted — DXY falling → long crypto

Usage:
  python scripts/run_macro_ablation.py
  python scripts/run_macro_ablation.py --config config/crypto_perps_full_rules.yaml
"""
...
_MACRO_RULES = ["macro_momentum_16", "macro_momentum_32", "dxy_momentum_16"]

_BASELINE = {"sharpe": 1.3983, "calmar": 1.9356, "maxdd": -0.0725}
...
def run_ablation(
    ...
    out_dir: str = "out/macro_ablation",
):
    ...
    print(f"\n{'='*70}")
    print(f"MACRO ABLATION RESULTS  (flat-63 baseline: "
...
def main():
    parser = argparse.ArgumentParser(description="Macro direction signal ablation backtests")
    ...
    parser.add_argument("--outdir", default="out/macro_ablation")

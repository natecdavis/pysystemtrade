#!/usr/bin/env python3
"""
Sweep skew_abs combined weight and compare backtest performance.

Runs a full backtest for each combined skew weight, collects metrics, and prints a
comparison table with adoption criteria check.

Signal: Carver (2020) time-series skew signal — 3 lookbacks (90d, 180d, 365d).
  Rolling log-return skew, pooled cross-asset z-score normalisation, negated.
  Instruments with more negative skew (vs cross-asset mean) outperform over 3–12 months.
  SR differential ≈ 0.4 at the 6-month horizon.
  NOT cross-sectional rank — each instrument gets its own time-series z-score.

Weight convention: equal weight shared across all three lookbacks.
  e.g. combined_weight=0.15 → skew_abs_90=0.05, skew_abs_180=0.05, skew_abs_365=0.05

Adoption criteria:
  ΔSharpe vs w=0.0  > +1%    (must add meaningful Sharpe)
  ΔMaxDD  vs w=0.0  < +3pp   (drawdown must not worsen by more than 3pp)
  Calmar should not sharply decline vs w=0.0

Usage:
    python scripts/sweep_skew_weight.py \\
        --base-config config/crypto_perps_full_rules.yaml \\
        --data data/dataset_538registry_6yr_jagged.parquet \\
        --outdir out/skew_sweep \\
...
# Weights to set per-rule (shared equally across 3 lookbacks)
SKEW_RULE_NAMES = ['skew_abs_90', 'skew_abs_180', 'skew_abs_365']
...
def print_comparison(results: list[dict]) -> None:
    ...
    print()
    print('=' * 110)
    print('SKEW-ABS COMBINED WEIGHT SWEEP — RESULTS')
    print('Signal: Carver (2020) rolling skew, pooled z-score, negated. 3 lookbacks: 90d / 180d / 365d.')
    print('Combined weight split equally: w_combined=0.15 → each rule gets 0.05.')
    print('Δ columns: relative to w_combined=0.0 (no-skew baseline)')
    print('=' * 110)
    ...
    if candidates:
        ...
        print(f'  RECOMMENDATION: Adopt skew_abs combined weight = {best_w:.2f}')
        print(f'    Per rule (90d/180d/365d): {best_each:.4f} each')
        print(f'    vs no-skew baseline:  ΔSharpe={d_s:+.1f}%,  ΔCalmar={d_c:+.4f},  ΔMaxDD={d_maxdd:+.2f}pp')
        print()
        print(f'  NEXT STEP: Update config/crypto_perps_full_rules.yaml:')
        print(f'    skew_abs_90:   {best_each:.4f}')
        print(f'    skew_abs_180:  {best_each:.4f}')
        print(f'    skew_abs_365:  {best_each:.4f}')
        print()
        print('  Then run one verification backtest to confirm, and commit.')
    else:
        print('  RECOMMENDATION: No weight passes all criteria — remove skew_abs rules from forecast_weights.')
        print('  If 180d/365d alone pass: ablate by dropping 90d first (higher noise).')
    print()


def main():
    parser = argparse.ArgumentParser(
        description='Sweep skew_abs combined weight.',
    ...
    parser.add_argument(
        '--outdir', type=Path,
        default=Path('out/skew_sweep'),
    )
    parser.add_argument(
        '--weights', type=float, nargs='+',
        default=[0.0, 0.03, 0.05, 0.08, 0.10, 0.15, 0.20],
        help='Combined skew weight (split equally across 3 rules). Default: 0.0 0.03 0.05 0.08 0.10 0.15 0.20',
    )
    parser.add_argument(
    ...
    print(f'Rules:          {SKEW_RULE_NAMES}  (equal share of combined weight)')
    print()
    ...
    for w_combined in args.weights:
        ...
        print(f'{"─" * 60}')
        print(f'Running: skew combined weight = {w_combined:.2f}  (each rule: {w_each:.4f})  →  {run_outdir}')
        ...
        # Ensure the skew rules are in trading_rules (they should be if base_cfg is full_rules.yaml)
        # Set per-rule weights in forecast_weights
        fw = dict(cfg.get('forecast_weights', {}))
        if w_combined < 1e-9:
            # Zero weight: remove skew rules from forecast_weights
            for rule in SKEW_RULE_NAMES:
                fw.pop(rule, None)
        else:
            for rule in SKEW_RULE_NAMES:

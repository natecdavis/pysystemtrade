#!/usr/bin/env python3
"""
Sweep illiquidity combined weight and compare backtest performance.

Runs a full backtest for each combined illiquidity weight, collects metrics, and prints a
comparison table with adoption criteria check.

Signal: Amihud (2002) illiquidity ratio — per-instrument time-series.
  |return| / dollar_volume, smoothed and z-scored. Instruments with higher illiquidity
  (vs their own history) expect an illiquidity premium — positive position bias.
  Two lookbacks: 20d and 60d. Both are implemented in rule_library.py.

  NOTE: The forecast weight estimator (rejected 2026-03-11) assigned ~12.7% combined weight
  to illiquidity_20/60. The estimator experiment was rejected (architecture mismatch), but
  the illiquidity signal deserves a direct, isolated weight sweep.

Weight convention: equal weight shared across both lookbacks.
  e.g. combined_weight=0.10 → illiquidity_20=0.05, illiquidity_60=0.05

Adoption criteria:
  ΔSharpe vs w=0.0  > +1%    (must add meaningful Sharpe)
  ΔMaxDD  vs w=0.0  < +3pp   (drawdown must not worsen by more than 3pp)
  Calmar: note separately (should not sharply decline)
  Turnover: flag if Carver cost filter would fail (SR/trade ≤ 0.01 or annual SR ≤ 0.13)

Usage:
    python scripts/sweep_illiquidity_weight.py \\
        --base-config config/crypto_perps_full_rules.yaml \\
        --data data/dataset_538registry_6yr_jagged.parquet \\
        --outdir out/illiquidity_sweep \\
...
ILLIQ_RULES = ['illiquidity_20', 'illiquidity_60']
...
def print_comparison(results: list[dict]) -> None:
    ...
    print()
    print('=' * 120)
    print('ILLIQUIDITY COMBINED WEIGHT SWEEP — RESULTS')
    print('Signal: Amihud (2002) |return|/dollar_volume, z-scored. 2 lookbacks: 20d / 60d.')
    print('Combined weight split equally: w_combined=0.10 → each rule gets 0.05.')
    print('Δ columns: relative to w_combined=0.0 (no-illiquidity baseline)')
    print('=' * 120)
    ...
    for r in results:
        ...
        if abs(w_combined - 0.0) < 1e-6:
            ...
            c1 = d_sharpe_pct > 1.0
            c2 = d_maxdd_pp < 3.0   # drawdown worsens by < 3pp (MaxDD is negative fraction)
    ...
    print('  Calmar: note separately (should not sharply decline)')
    ...
    if candidates:
        ...
        print(f'  RECOMMENDATION: Adopt illiquidity combined weight = {best_w:.2f}')
        print(f'    Per rule (20d/60d): {best_each:.4f} each')
        print(f'    vs no-illiquidity baseline:  ΔSharpe={d_s:+.1f}%,  ΔCalmar={d_c:+.4f},  ΔMaxDD={d_maxdd:+.2f}pp')
        print()

        # Carver cost filter check on best candidate
        ...
        print(f'    illiquidity_20:  {best_each:.4f}')
        print(f'    illiquidity_60:  {best_each:.4f}')
        print()
        print('  Then run one verification backtest to confirm, and commit.')
    else:
        print('  RECOMMENDATION: No weight passes all criteria — keep illiquidity_20/60 at forecast_weight=0.0.')
...
def main():
    parser = argparse.ArgumentParser(
        description='Sweep illiquidity_20/60 combined weight.',
    ...
    parser.add_argument(
        '--outdir', type=Path,
        default=Path('out/illiquidity_sweep'),
    )
    parser.add_argument(
        '--weights', type=float, nargs='+',
        default=[0.0, 0.03, 0.05, 0.08, 0.10, 0.15, 0.20],
        help='Combined illiquidity weight (split equally across 2 rules). Default: 0.0 0.03 0.05 0.08 0.10 0.15 0.20',
    )
    parser.add_argument(
    ...
    print(f'Rules:          {ILLIQ_RULES}  (equal share of combined weight)')
    print()
    ...
    for w_combined in args.weights:
        ...
        print(f'{"─" * 60}')
        print(f'Running: illiquidity combined weight = {w_combined:.2f}  (each rule: {w_each:.4f})  →  {run_outdir}')
        ...
        if w_combined < 1e-9:
            # Zero weight: remove illiquidity rules from forecast_weights (true baseline)
            for rule in ILLIQ_RULES:
                fw.pop(rule, None)
        else:
            for rule in ILLIQ_RULES:

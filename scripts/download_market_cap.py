#!/usr/bin/env python3
"""
Download daily market cap (CapMrktCurUSD) from CoinMetrics Community API.

Source: https://community-api.coinmetrics.io/v4/timeseries/asset-metrics
        ?assets={comma-joined}&metrics=CapMrktCurUSD&frequency=1d&page_size=10000

CapMrktCurUSD = current market capitalisation in USD (circulating supply × price).
Used as denominator in the C-5 VAL factor: AdrActCnt / CapMrktCurUSD.

Coverage: same 42 instruments as active_addresses.parquet (INSTR_TO_CM mapping is
shared). BTC, ETH, ADA, AVAX, LINK, DOGE, XRP, LTC, BCH, AAVE, UNI, SUSHI, CRV,
COMP, SNX, YFI, LDO, UMA, ZEC, ZRX, VET, KNC, TRX, MANA, and others.

Lit: Cong et al. (2022) C-5 VAL — AdrActCnt/MarketCap = crypto book-to-market.
     High ratio (many addresses per $ of market cap) → undervalued relative to peers.

Output: data/market_cap.parquet
    Index: DatetimeIndex (tz-naive UTC midnight, daily)
    Columns: Binance instrument codes (BTCUSDT_PERP, ETHUSDT_PERP, ...)
    Values: float64 (daily market cap in USD, NaN where not available)

Usage:
    python scripts/download_market_cap.py
    python scripts/download_market_cap.py --output data/market_cap.parquet
"""
...
def fetch_batch(cm_assets: list, start_time: str = "2020-01-01") -> list:
    """
    Fetch CapMrktCurUSD for a batch of CoinMetrics asset IDs.

    CoinMetrics allows comma-joined assets in a single request.
    Returns list of API records: [{asset, time, CapMrktCurUSD}, ...]
    """
    assets_str = ",".join(cm_assets)
    base_url = (
        f"{CM_BASE}?assets={assets_str}"
        f"&metrics=CapMrktCurUSD"
        f"&frequency=1d"
...
def fetch_all_market_caps(start_time: str = "2020-01-01") -> list:
    """
    Fetch CapMrktCurUSD for all instruments in INSTR_TO_CM.
    ...
    print(f"Fetching CapMrktCurUSD for {len(cm_assets)} CoinMetrics assets in batches ...")
...
def parse_records(records: list) -> pd.DataFrame:
    ...
    Input records format:
        {"asset": "btc", "time": "2020-01-01T00:00:00.000000000Z", "CapMrktCurUSD": "12345678"}
    ...
    for rec in records:
        ...
        cap_str = rec.get("CapMrktCurUSD")

        # Skip records without coverage
        if not cap_str or cap_str in ("", "null", None):
            skipped += 1
            continue
    ...
    wide_df = long_df.pivot(index="date", columns="instrument", values="CapMrktCurUSD")
...
def print_coverage_summary(df: pd.DataFrame) -> None:
    ...
    print()
    print("=" * 72)
    print("MARKET CAP COVERAGE SUMMARY")
    print("=" * 72)
...
def main():
    parser = argparse.ArgumentParser(
        description="Download market cap (CapMrktCurUSD) from CoinMetrics Community API",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument(
        "--output",
        default="data/market_cap.parquet",
        help="Output path for parquet file (default: data/market_cap.parquet)",
    )
    parser.add_argument(
        "--start-time",
        default="2020-01-01",
    ...
    print(f"Downloading CapMrktCurUSD from CoinMetrics Community API ...")
    print(f"  Instruments: {len(INSTR_TO_CM)} ({len(set(INSTR_TO_CM.values()))} unique CM assets)")
    print(f"  Start: {args.start_time}")
    print(f"  Output: {args.output}")
    print()

    records = fetch_all_market_caps(start_time=args.start_time)
